...
function* getAllStudentsWorker() {
  try {
    const students: any[] = yield call(async () => await GET(STUDENT_BASE_URL, true));
...
function* getStudentByIdWorker({ payload }: any) {
  try {
    const student: object = yield call(async () => await GET(`${STUDENT_BASE_URL}?id=${payload}`, true));
...
function* createStudentWorker({ payload }: any) {
  try {
    const response: object = yield call(async () => await POST(STUDENT_BASE_URL, payload, true));
...
function* updateStudentWorker({ payload }: any) {
  try {
    ...
    const response: object = yield call(async () => await PUT(`${STUDENT_BASE_URL}?id=${id}`, rest, true));
...
function* deleteStudentWorker({ payload }: any) {
  try {
    const response: object = yield call(async () => await DELETE(`${STUDENT_BASE_URL}?id=${payload}`, true));
